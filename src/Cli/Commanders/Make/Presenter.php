...
class Presenter extends Make
{
     ...
     * @var string
     */
    protected $commandDescription = 'DESC_CLI_MAKE_PRESENTER';
    ...
    public function execute()
    {
        ...
        if ( empty( $this->optionFilename ) ) {
            output()->write(
                ( new Format() )
                    ...
                    ->setString( language()->getLine( 'E_CLI_PRESENTER_FILENAME' ) )
                    ->setNewLinesAfter( 1 )
            );
        ...
        if ( is_file( $filePath ) ) {
            output()->write(
                ( new Format() )
                    ->setContextualClass( Format::DANGER )
                    ->setString( language()->getLine( 'E_CLI_PRESENTER_EXISTS', [ $filePath ] ) )
                    ->setNewLinesAfter( 1 )
            );
        ...
        if ( is_file( $filePath ) ) {
            output()->write(
                ( new Format() )
                    ...
                    ->setString( language()->getLine( 'S_CLI_PRESENTER_MAKE', [ $filePath ] ) )
                    ->setNewLinesAfter( 1 )
            );
