...
class Has
{
    ...
    protected function setRelationship( $relation )
    {
        // Try to load reference model
        $relationModel = $this->_loadRelationModel( $relation );

        $relationship = new \stdClass();

        if ( $relationModel instanceof Model ) {
            $relationship->model = $relationModel;
            $relationship->table = $relationModel->table;
        } else {
